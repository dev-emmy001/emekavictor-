...
const testimonials = [
  {
    ...
    bg_color: "group-hover:bg-orange-950/50",
  ...
  },
  {
    ...
    bg_color: "group-hover:bg-emerald-950/50",
  ...
  },
  {
    ...
    bg_color: "group-hover:bg-blue-950/50",
  ...
  },
  {
    ...
    text_color: "group-hover:text-gray-200",
    bg_color: "group-hover:bg-gray-800",
    quote_color: "text-gray-400"
  }
];
...
export default function Testimonials() {
  ...
  return (
    <section className="py-24 bg-gray-950 overflow-hidden cursor-default">
      <div className="mx-auto px-6 md:px-8">
        
        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div className="max-w-2xl">
            <h2 className="text-4xl md:text-6xl font-bold tracking-tighter text-gray-100 mb-6">
              Don't just take <br />
          ...
          >
            {testimonials.map((t) => (
              <motion.div 
                key={t.id}
                className={`
                  group min-w-[320px] md:min-w-[500px] p-8 md:p-12 rounded-[2rem] h-fit bg-gray-900 
                  border border-gray-800 transition-all duration-500 
                  ${t.color} hover:bg-gray-800 hover:shadow-2xl hover:shadow-black/50
                `}
              >
                {/* Quote Icon */}
                <div className="mb-8">
                  <Quote 
                    size={48} 
                    className={`text-gray-700 transition-colors duration-500 ${t.text_color} fill-current opacity-20`} 
                  />
                </div>

                {/* Text */}
                <p className="text-l md:text-xl font-medium text-gray-300 leading-relaxed mb-10">
                  {t.text}
                </p>
                ...
                <div className="flex items-center gap-4">
                  {/* Initials Avatar */}
                  <div className={`w-12 h-12 rounded-full bg-gray-800 flex items-center justify-center text-gray-400 font-bold transition-colors duration-500 ${t.bg_color} ${t.text_color}`}>
                    {t.name.charAt(0)}
                  </div>
                  
                  <div>
                    <h4 className={`font-bold text-gray-100 transition-colors duration-500 ${t.text_color}`}>
                      {t.name}
                    </h4>
